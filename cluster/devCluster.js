...
if (program.host && program.template && program.count) {
    ...
    dns.lookup(host, function onLookup(e, ip, family) {
        dns.lookup(hosts[1], function onLookup(e, ip2, family) {
            ...
            var fC = function() {
                var count = new vz.supervisors.Count(Cluster, vz.containers[program.template], program.count, false);
                var cHosts = count.getHostsSortByCtnCount();
                console.log(c.green.bgBlack('Count Host', cHosts[0].containers.length));
                var Host = _.pick(cHosts[0], hostFilter);
                var VMs = cHosts[0].containers.map(function(c) {
                    return _.pick(c, vmFilter);
                });

                console.log('VMs', pj.render(VMs.length));
                console.log('Host', pj.render(Host.length));
