var SSHHost = require('./sshHost').SSHHost, Container = require('./container').Container, htmlDecode = require('./encoder').htmlDecode, log = require('czagenda-log')
        ...
        var c = require('chalk'), pj = require('prettyjson'), _ = require('underscore');
...
Host.prototype._refreshContainers = function() {
	...
	h.ssh(BIN_VZINFO, {
				...
				res = JSON.parse(res);
                    ...
                    console.log(e);
                    console.log(c.red.bgBlack('Unable to parse result of ', res));
                    return;
                    process.exit();
                }
				var containers = []
...
Host.prototype.vzctl = function(command, options, callback) {
	var h = new SSHHost(this.ip, this.user);
    options = options || {};
    var BIN = options.BIN || '/usr/sbin/vzctl';
	h.ssh(BIN + ' ' + command.join(' '), options, function(err, res, code) {
