...
//    console.log('info', info,'host', host);
	this.description = info.description;

    console.log('desc', info.description);
//    this.description = encoder.htmlDecode(this.description);
...
//    console.log('desc', info.description);
	...
	this.ipMonitor = null;
	this.exec_queue = async.queue(this._exec.bind(this), 1);
...
Container.prototype.saveDescription = function(callback) {
	...
	this.host.vzctl([
					...
					this.id,
					'--description',
//					'"' + encoder.htmlEncode(JSON.stringify(this.description))+'"', '--save'],
					'"' + JSON.stringify(this.description)+'"', '--save'],
//					'"'+JSON.stringify(this.description)+'"', '--save'],
//					'"'+encoder.htmlEncode(JSON.stringify(this.description))+'"', '--save'],
//					'"{"type": "' + this.description.type + '"}"',' --save'],
                            null, callback);
}
	...
	async.series({
		...
		'create' : function(callback) {
			log.info('Start creating new container with template '
					+ this.ostemplate);
			this.host.vzctl(
					['create', this.id, '--ostemplate', this.ostemplate], {vzctlJS: true},
